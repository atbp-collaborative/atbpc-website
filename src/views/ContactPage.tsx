...
interface ContactPageProps {
  section: 'contact' | 'case-study-house' | 'grow-with-us' | 'partner-with-us' | 'contact-info';
}
...
export const ContactPage: React.FC<ContactPageProps> = ({ section }) => {
      ...
      <div className="flex-1 flex flex-col justify-center w-full min-h-0 h-full overflow-hidden">
        <div className="flex flex-col md:flex-row gap-[2px] flex-1 h-full min-h-0 items-stretch group/cardsContainer">
          {cards.map((card) => (
            <div
              key={card.id}
              onClick={() => router.push(TAB_TO_ROUTE[card.tab] ?? ROUTES.contact)}
