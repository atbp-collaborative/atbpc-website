  ...
  return (
    <div className="w-full h-full select-none flex flex-col flex-1 min-h-0 overflow-hidden">
      <div className="flex-1 flex flex-col justify-center w-full min-h-0 h-full overflow-hidden">
        <div className={`flex gap-[2px] flex-1 h-full min-h-0 items-stretch group/cardsContainer ${
          ...
          {cards.map((card) => (
            <div
              ...
              className={`group/card relative flex-1 min-h-0 w-full overflow-hidden rounded-none border-0 cursor-pointer transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover/cardsContainer:opacity-75 hover:!opacity-100 ${
                layout === 'row' ? 'hover:flex-[1.45]' : ''
              }`}
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8 md:p-10 flex flex-col justify-end items-start text-left z-10">
                ...
                <h2 className="font-sans text-h2 sm:text-h1 font-semibold tracking-tight text-white lowercase drop-shadow-md">
                  {card.title}
                </h2>
