...
interface MultiContactModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (contacts: PartnerContact[]) => void;
  initialContacts: PartnerContact[];
  isDarkMode: boolean;
}

export const MultiContactModal: React.FC<MultiContactModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialContacts,
  ...
  useEffect(() => {
    if (isOpen) {
      setContacts(initialContacts.length > 0 && initialContacts[0].number ? initialContacts : [{ number: '', description: '' }]);
    }
  ...
  const handleUpdate = (index: number, patch: Partial<PartnerContact>) => {
  ...
  const handleAdd = () => {
    if (contacts.length < 5) {
      setContacts(prev => [...prev, { number: '', description: '' }]);
    }
  };
  ...
  const isSaveDisabled = contacts.some(c => !c.number || !c.description);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10">
          ...
          <motion.div
            ...
            className={`relative w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl border z-10 overflow-hidden ${
          ...
          >
            <div className={`flex items-center justify-between px-6 py-5 border-b sticky top-0 z-20 backdrop-blur-md ${isDarkMode ? 'bg-vintage-charcoal/95 border-space-sparkle/20' : 'bg-white/95 border-space-sparkle/15'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-h2 font-sans font-bold tracking-tight">Contact Numbers</h2>
                  <p className="text-mini opacity-60 font-sans uppercase tracking-wider">Add up to 5 numbers</p>
                </div>
              </div>
                ...
                <div key={idx} className="flex gap-4 items-start relative">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                    <TextField
                      ...
                      placeholder="e.g. Viber & Whatsapp Ready"
                      onChange={(_, val) => handleUpdate(idx, { number: val })}
